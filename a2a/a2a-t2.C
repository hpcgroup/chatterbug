  ...
  for (i = 0; i < MAX_ITER; i++) {
    ...
    for(int zcoord = 1; zcoord < dims[MP_Z]; zcoord++) {
      MPI_Isend(&sendbuf[off], perrank, MPI_CHAR,
        ...
        MPI_COMM_WORLD, &sreq[zcoord]);
#if CMK_BIGSIM_CHARM
      changeMessage(timeLine[timeLine.length() - 3]);
#endif
      MPI_Irecv(&recvbuf[off], perrank, MPI_CHAR,
        calc_pe(myXcoord, myYcoord, (zcoord + myZcoord) % dims[MP_Z]), 0,
        MPI_COMM_WORLD, &rreq[zcoord]);
      off += perrank;
    }
    MPI_Waitall(dims[MP_Z] - 1, &sreq[1], MPI_STATUSES_IGNORE);
    MPI_Waitall(dims[MP_Z] - 1, &rreq[1], MPI_STATUSES_IGNORE);
#if CMK_BIGSIM_CHARM
    BgMark("FFTTime");    
#endif
  }
